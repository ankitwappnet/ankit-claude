...
class ModuleElementsUtils {
	...
	public static function get_responsive_image_attrs( array $image_attr_value ): array {
		...
		// Only strings may be passed to attachment_url_to_id(); non-strings are ignored for URL resolution.
		$image_src = '';
		if ( isset( $image_attr_value['src'] ) && is_string( $image_attr_value['src'] ) ) {
			$image_src = $image_attr_value['src'];
		} elseif ( isset( $image_attr_value['url'] ) && is_string( $image_attr_value['url'] ) ) {
			$image_src = $image_attr_value['url'];
		}
